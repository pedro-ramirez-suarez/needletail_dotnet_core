...
namespace Generator
{
    class Program
    {
        ...
        //    return Assembly.Load(new AssemblyName (di.FullName + @"\" + dll[0] + ".dll"));
        //}
        ...
        static void GenerateModel()
        {
            ...
            if (entityName.ToLower() == "fulldb")
            {
                ...
                try
                {
                    ...
                    using (cn = new SqlConnection(cString))
                    {
                        cn.Open();
        ...
        static void WriteEntity(Type entityType, XmlWriter writer, string customAttrName, string customAttrVal, string name, bool skipElements = false)
        {
            ...
            if (!skipElements)
            {
                ...
                if (key.PropertyType.Namespace != "System" && key.PropertyType.GetTypeInfo().IsClass)
            ...
            foreach (var p in props)
            {
                ...
                if (p.PropertyType.GetTypeInfo().IsClass && p.PropertyType.Name != "String")
                {
